...
class _DayPickerState extends State<_DayPicker> {
  ...
  Widget build(BuildContext context) {
    ...
    final int totalDaySlots = widget.dayRowsCount * DateTime.daysPerWeek;
    final bool showPreviousOutsideDays =
        widget.config.showPreviousOutsideDays == true;
    final bool showNextOutsideDays = widget.config.showNextOutsideDays == true;

    for (int slot = 0; slot < totalDaySlots; slot++) {
      final int dayNumber = slot - dayOffset + 1;
      if (dayNumber < 1) {
        if (!showPreviousOutsideDays) {
          dayItems.add(Container());
          continue;
        }
      } else if (dayNumber > daysInMonth) {
        if (!showNextOutsideDays) {
          break;
        }
      }

      final DateTime dayToBuild = DateTime(year, month, dayNumber);
      final bool isOutside = dayToBuild.month != month;
      ...
      if (isSelectedDay) {
        ...
        dayColor = widget.config.outsideDaysColor ?? disabledDayColor;
      }
      ...
      if (isOutside && !isSelectedDay && !isDisabled) {
        final outsideColor = widget.config.outsideDaysColor ?? disabledDayColor;
        customDayTextStyle = (customDayTextStyle ?? dayStyle).copyWith(
          color: outsideColor,
        );
      }
      ...
      if (isDisabled) {
        ...
        dayWidget = InkResponse(
          focusNode: isOutside ? null : _dayFocusNodes[dayNumber - 1],
